...
public class ProcessBuilderExecutorTest extends AbstractProcessBuilderExecutorTest {
	...
	public void syncExecute() throws Exception {
		final int result = ProcessBuilderExecutor.syncExecute(processBuilder);
		assertEquals("Response code should be 0. ", 0, result);
	}

	@Test
	public void syncExecuteWithOutputFile() throws Exception {
		final File file = getTemporaryFolder().newFile();
		final int result = ProcessBuilderExecutor.syncExecute(processBuilder, file);
		assertEquals("Response code should be 0. ", 0, result);
		assertTrue("File with output does not exists. ", file.exists());
		final String fileContent = fileContentToString(file);
		assertThat(fileContent, is(equalToIgnoringWhiteSpace(EXPECTED_RESULT)));
	}

	@Test
	public void asyncExecute() throws Exception {
		final Process process = ProcessBuilderExecutor.asyncExecute(processBuilder);
		process.waitFor();
	...
	public void asyncExecuteWithOutputFile() throws Exception {
		final File file = getTemporaryFolder().newFile();
		final Process process = ProcessBuilderExecutor.asyncExecute(processBuilder, file);
		process.waitFor();
		log.debug("File content: {}", fileContentToString(file));
		assertEquals("Response code should be 0. ", 0, process.exitValue());
		assertTrue("File with output does not exists. ", file.exists());
		final String fileContent = fileContentToString(file);
		assertThat(fileContent, is(equalToIgnoringWhiteSpace(EXPECTED_RESULT)));
	...
	private String fileContentToString(final File file) throws IOException {
